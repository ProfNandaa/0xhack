...
def get_last_input():
	...
	# row determine how long the machine has been running
	run_time = kernel32.GetTickCount()

	alapsed = run_time - struct_lastinputinfo.dwTime
...
def get_key_press():
	...
	for i in range(0, 0xff):
		...
		# 0x1 is the code for a left mouse-clicks
		if i == 0x1:
			mouse_clicks += 1
			return time.time()
		elif i > 32 and i < 127:
...
def detect_sandbox():
	...
	while not detection_complete:
		...
		if keypress_time is not None and previous_timestamp is not None:
			# calclulate the time between double clicks
			elapsed = keypress_time - previous_timestamp

			# the user doubled clicked
			...
			if keystrokes >= max_keystrokes and double_clicks >= max_double_clicks \
